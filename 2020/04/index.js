...
async function part1 () {
  ...
  passports.forEach((passport) => {
    ...
    if (required.every((key) => keys.includes(key))) answer1++;
  });
...
function validate(field, value) {
  return ({
    ...
    'hgt': (value) => {
      const type = (value.match(/cm|in/g) || [])[0];
      const num = (value.match(/\d+/g) || [])[0];
    ...
    'hcl': (value) => /^#([0-9a-f]{6})/g.test(value),
    'ecl': (value) => /amb|blu|brn|gry|grn|hzl|oth/g.test(value),
...
async function part2 () {
  ...
  passports.forEach((passport) => {
    ...
    fields.forEach((field) => {
      ...
      if (key === 'cid') return;
      if (validate(key, value)) validField++;
    });
